...
class StudentGuide extends React.Component{ 
    ...
    async componentDidMount(){
        ...
        await this.props.getLinks('Student')
        this.setState({loading : false})
    }

    render(){    
        if(this.state.loading) return loading('tw-w-12 tw-text-grayish centerize')
        return(
...
const mapStateToProps = state =>{
    return{
        link : state.guide.studentLinks
    }
}

const cwrapped = connect(mapStateToProps , {getLinks})(StudentGuide)
