...
class TeacherGuide extends React.Component{ 
    ...
    async componentDidMount(){
        ...
        await this.props.getLinks('Teacher')
        this.setState({loading : false})

    }

    render(){    
        if(this.state.loading) return loading('tw-text-grayish tw-w-12 centerize')
        return(
            <div className="tw-w-screen tw-px-4 tw-min-h-screen tw-bg-black-blue tw-flex md:tw-flex-row tw-flex-col-reverse tw-justify-evenly tw-items-center">
                ...
                <div className="tw-text-center">
                    ...
                    <p className="tw-text-white tw-py-6 tw-text-lg">{this.props.t('teachersGuideVideo')}</p>
...
const mapStateToProps = state =>{
    return{
        link : state.guide.teacherLinks
    }
}

const cwrapped = connect(mapStateToProps , {getLinks})(TeacherGuide)
