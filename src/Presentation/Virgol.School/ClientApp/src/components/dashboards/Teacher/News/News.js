        ...
        return (
            <div className="tw-w-full tw-mt-10">
                ...
                <PlusTable
                    ...
                    button={() => {
                        return (
                        <button onClick={() => history.push('/addNewsTeacher')} className="tw-px-6 tw-py-1 tw-border-2 tw-border-sky-blue tw-text-sky-blue tw-rounded-lg">{this.props.t('addNews')}</button>
                        );
                    }}
                                    ...
                                    this.props.myNews.map(x => {
                                        if(x.message.includes(this.state.query))
                                        {
                                            ...
                                            <tr key={x.id}>
                                                ...
                                                <td className="tw-cursor-pointer tw-px-4 tw-py-4" onClick={() => history.push(`/teacherNews/${x.id}`)}>
                                                    {edit('tw-w-6 tw-text-white')}
                                                </td>
                                                <td onClick={() => this.showDelete(x.id)} className="tw-cursor-pointer tw-px-4 tw-py-4">
