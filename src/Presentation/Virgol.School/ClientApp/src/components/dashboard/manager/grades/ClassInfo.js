...
class ClassInfo extends React.Component {
    ...
    async componentDidMount() {
            ...
            await this.props.getClassInGrades(this.props.token, parseInt(this.props.match.params.id));
            this.setState({loading: false})
    }


    renderCourses = () => {
        return this.props.classes.map((classs, i) => {
            return (
                <CourseCard
                    key={classs.id}
                    course={classs}
    ...
    deleteClass = (id) => {
        ...
        this.props.deleteClass(this.props.token, id)
    }
                ...
                <div className="md:w-8/12 md:mb-0 mb-8 w-11/12 min-h-90h flex flex-col justify-between">
                    <div className="w-full flex flex-row justify-between items-center">
                        ...
                        <div className="flex flex-row-reverse items-center justify-center">
                            <span className="text-dark-blue text-2xl">تست</span>
...
const mapStateToProps = (state, ownProps) => {
    ...
    return { token: state.auth.userInfo.token, classes : state.managerData.classes }
}

const authWrapped = protectedManager(ClassInfo)
export default connect(mapStateToProps, { getClassInGrades, addNewClass, editClass, deleteClass })(authWrapped);