...
import { addBulkUser } from "../../../../_actions/managerActions";
import history from "../../../../history";
import {clear, excel, loading} from "../../../../assets/icons";

class AddStudents extends React.Component {

    state = { dragging : false, file: null, loading: false }
    ...
    submit = async () => {
        if (this.state.file != null && !this.state.loading) {
            ...
            await this.props.addBulkUser(this.props.token, this.state.file);
        ...
        return (
            <div className="w-screen min-h-screen bg-green-800 flex justify-center items-center">
                <div className="max-w-1000 py-12 relative w-5/6 bg-white flex flex-col justify-center items-center">
                    ...
                    <button onClick={this.submit} className={`mt-4 bg-golden text-dark-green transition-all duration-200 font-vb text-xl px-16 py-2 focus:outline-none focus:shadow-outline rounded-lg ${this.state.file === null ? 'invisible' : 'visible'}`}>
                        {this.state.loading ? loading("w-6 text-black") : 'افزودن'}
                    </button>
                </div>
            </div>
        );
...
export default connect(mapStateToProps, { addBulkUser })(AddStudents);