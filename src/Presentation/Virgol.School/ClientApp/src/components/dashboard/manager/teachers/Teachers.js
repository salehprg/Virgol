...
import { getAllTeachers, addNewTeacher, fadeError, deleteTeacher } from "../../../../_actions/managerActions";
import {
    addPerson,
    clear, edit,
    errorOutline, excel,
                            ...
                            <tr key={teacher.id}>
                                ...
                                <td className="flex flex-col items-center py-2">
                                    <div className="flex flex-row justify-center">
                                        <div onClick={() => history.push(`/m/teacher/${teacher.id}`)} data-tip="ویرایش">
                ...
                {this.state.renderAddTeacherModal ?
                    <Modal cancel={this.onCancelAdd}>
                        <div onClick={(e) => e.stopPropagation()} className="lg:w-3/5 md:w-4/5 w-5/6 py-16 px-4 relative flex md:flex-row  flex-col items-center justify-center bg-white font-vb">
                            ...
                            <div onClick={() => history.push("/m/addTeacher")} className="flex md:flex-col flex-row md:h-64 h-32 py-4 px-2 justify-center items-center md:w-1/4 w-5/6 md:mx-8 mx-0 md:my-0 my-4 border-green-800 border-4 cursor-pointer transform duration-200 hover:scale-110">
                                {addPerson("w-1/2 text-green-800")}
                                <span className="text-2xl text-center text-green-800 mt-4">افزودن به صورت دستی</span>
                            </div>
                            <div onClick={() => history.push("/m/addTeacherByExcel")} className="flex md:flex-col flex-row md:h-64 h-32 py-4 px-2 justify-center items-center md:w-1/4 w-5/6 md:mx-8 mx-0 md:my-0 my-4 border-green-800 border-4 cursor-pointer transform duration-200 hover:scale-110">
