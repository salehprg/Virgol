...
import { getAllCategory, addNewCategory, deleteCategory, fadeError, wipeCatInfo } from "../../../../_actions/managerActions";
        ...
        if (categories !== null) {
            ...
            const cards = categories.map((category) => {
                if (category.name.includes(this.state.query)) {
                    return (
                        <div key={category.id} className="w-5/6 max-w-200 sm:ml-12 ml-0 mb-12 py-4 bg-white flex flex-col items-center">
                            <span className="text-xl text-blueish font-vb text-center">{category.name}</span>
                            <span className="my-4 text-center" dir="rtl"></span>
                            <Link className="w-5/6 py-2 bg-dark-blue text-center text-white rounded-full" to={`/m/cat/${category.id}`}>ویرایش</Link>
                        </div>
                    );
                }
            })
...
const mapStateToProps = (state) => {
    return {
        ...
        categories: state.managerData.categories,
        categoryCourses: state.managerData.categoryCourses
    }
}
